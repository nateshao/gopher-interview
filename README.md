...
1. **值传递**：值传递是将变量的副本传递给函数。在函数内部修改该副本，不会影响到原始变量。
...
原文：http://golang.design/go-questions/slice/vs-array/

1. **数组与切片的定义与特性**

*数组（Array）**

- **定义**：数组是固定长度的序列，一旦定义，长度就不可改变。数组是值类型，这意味着在赋值或传递时，会创建数组的副本。
- **特性**：由于数组是值类型，所以在进行赋值或传递给函数时，会产生一份完整的拷贝，修改副本不会影响原始数组。

```go
var arr [5]int // 定义一个长度为5的整数数组
arrCopy := arr // 赋值操作，创建 arr 的副本
arrCopy[0] = 10
fmt.Println(arr[0]) // 输出 0，原数组未被修改
```

*切片（Slice）**

- **定义**：切片是基于数组的动态序列，可以根据需要动态调整长度。切片是引用类型，指向底层数组，切片本身并不存储数据，而是通过一个指针、长度和容量来管理底层数组。
- **特性**：切片在赋值或传递时，新的切片仍然引用同一个底层数组，因此对切片的修改会影响到其他引用相同底层数组的切片。

```go
s := []int{1, 2, 3} // 定义一个切片
sCopy := s          // 赋值操作，sCopy 和 s 共享相同的底层数组
sCopy[0] = 10
fmt.Println(s[0]) // 输出 10，原切片受到了影响
```
...
数组在内存中是连续存储的，其大小在编译时就已经确定。由于数组是值类型，每次传递或赋值都会复制整个数组。

- **优点**：由于内存布局连续，访问速度非常快，适合需要高性能的场景。
- **缺点**：灵活性不足，长度固定，且在传递大数组时会有较高的性能开销。

**切片**

切片的底层实现是一个 `slice` 结构体，包含指向数组的指针、切片的长度和容量。它的灵活性来源于可以动态调整长度，且只引用底层数组，不复制数据。
...
- **优点**：切片非常灵活，支持动态调整大小，且由于是引用类型，赋值和传递的性能开销较小。
- **缺点**：由于多个切片可以引用同一个底层数组，容易引发意外的修改，此外，当切片需要扩容时，可能会导致内存重新分配，带来性能开销。

3. **性能和使用场景分析**

**数组**

- **性能**：由于是值类型，数组在传递时会复制整个数据结构，适合小型、固定长度的场景，但在处理大型数据时性能较低。
- **使用场景**：数组适用于需要高性能访问且长度固定的场景，如处理小型集合、矩阵计算等。

**切片**

- **性能**：切片由于是引用类型，传递和赋值的性能较高，但在频繁扩容时可能引发性能问题。
- **使用场景**：切片广泛用于需要灵活处理动态数据的场景，如处理大型集合、动态列表等。

4. **示例与分析**

通过以下代码可以直观地理解数组和切片的行为差异：
...
得出结论：数组是值类型，适合处理固定大小的数据集，而切片是引用类型，更适合处理动态数据。



## 1.4 Golang中[切片的容量是怎样增长的](http://golang.design/go-questions/slice/grow/)

2. **切片扩容的触发**

当我们向切片追加元素且长度超过当前容量时，Go 会自动为切片扩容。这个过程涉及分配一个更大的底层数组，并将现有数据复制到新数组中。这个扩容的过程是切片增长的关键。

3. **切片容量的增长策略**

Go 语言的切片扩容策略是根据切片当前容量的大小进行动态调整的。在源码中，这个策略体现在 `runtime.growslice` 函数中。扩容的增长规律如下：

- 当切片的容量小于 1024 时，切片的容量会按照 **倍数** 增长，即每次扩容时容量会翻倍。
...
从这个示例中可以看到，随着元素的增加，切片的容量按照倍数增长。当元素数目达到 8 时，容量翻倍为 16，符合之前提到的增长策略。

[golangSlice的扩容规则](https://jodezer.github.io/2017/05/golangSlice的扩容规则)：https://jodezer.github.io/2017/05/golangSlice%E7%9A%84%E6%89%A9%E5%AE%B9%E8%A7%84%E5%88%99
